    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Topic: ${topic}\n\nExtract key facts, prioritizing USABILITY info:\n- What is the tool/feature?\n- How do you access it? (URL, app, extension, etc.)\n- What specific steps does a user take?\n- What's the result/payoff?\n\nArticles:\n${articlesContext}\n\nReturn JSON: {"facts": [{"fact": "specific factual claim or instruction", "sourceUrl": "article URL", "sourceIndex": 1}]}\n\nRULES:\n- Extract 5-12 facts\n- Prioritize how-to steps and accessibility info\n- Each fact must be in at least one source\n- Do NOT add claims not in the articles`,
          },
        ],
        temperature: 0.2,
        max_completion_tokens: 2000,
